...
def export(args):
    ...
    hits = run_query(mozart_es_url, "containers", {"query": {"term": {"_id": cont_id}}})
    if len(hits) == 0:
    ...
    job_specs = [
        i["_source"]
        for i in run_query(
            mozart_es_url, "job_specs", {"query": {"term": {"container.raw": cont_id}}}
        )
    ]
    ...
    if len(job_specs) == 0:
        ...
        job_specs = [
            ...
            for i in run_query(
                ...
                {
                    "query": {
                        "query_string": {"query": 'container:"{}"'.format(cont_id)}
                    }
                },
            )
        ]
    ...
    for job_spec in job_specs:
        ...
        mozart_hysds_ios = [
            ...
            for i in run_query(
                mozart_es_url,
                "hysds_ios",
                {"query": {"term": {"job-specification.raw": job_spec["id"]}}},
            )
        ]
        logger.debug(
            "Found {} hysds_ios on mozart for {}.".format(
        ...
        logger.debug(
            "Found {} hysds_ios on grq for {}.".format(
    ...
    # dump manifest JSON
    manifest = {"containers": cont_info, "job_specs": job_specs, "hysds_ios": hysds_ios}
...
def import_pkg(args):
    ...
    for hysds_io in manifest["hysds_ios"]:
        ...
        r = requests.put(
            "{}/hysds_ios/hysds_io/{}".format(es_url, hysds_io["id"]),
