...
# all verdi hosts
verdi_hosts = ["%s" % context["VERDI_PVT_IP"]]
if context.get("OTHER_VERDI_HOSTS", None) is not None:
    verdi_hosts.extend(
        [
            i["VERDI_PVT_IP"]
            for i in context["OTHER_VERDI_HOSTS"]
            if i["VERDI_PVT_IP"] is not None
        ]
    )
...
if not os.path.isfile(env.key_filename):
    raise RuntimeError(
        "SSH key filename %s doesn't exist. " % env.key_filename
...
def set_spyddder_settings():
    upload_template(
        "settings.json.tmpl",
        "~/verdi/ops/spyddder-man/settings.json",
        use_jinja=True,
        context=get_context(),
        template_dir=os.path.join(ops_dir, "mozart/ops/spyddder-man"),
    )
...
def rsync_code(node_type, dir_path=None):
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/osaka"),
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/hysds_commons"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/hysds"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/prov_es"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/sciflo"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/chimera"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/container-builder"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/lightweight-jobs"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    ...
    rsync_project(
        "%s/ops/" % dir_path,
        os.path.join(ops_dir, "mozart/ops/hysds-dockerfiles"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
    if node_type in ("mozart"):
        ...
        rsync_project(
            "%s/ops/" % dir_path,
            os.path.join(ops_dir, "mozart/ops/mozart"),
            extra_opts=extra_opts,
            ssh_opts=ssh_opts,
        )
        ...
        rsync_project(
            "%s/ops/" % dir_path,
            os.path.join(ops_dir, "mozart/ops/figaro"),
            extra_opts=extra_opts,
            ssh_opts=ssh_opts,
        )
    if node_type == "verdi":
        ...
        rsync_project(
            "%s/ops/" % dir_path,
            os.path.join(ops_dir, "mozart/ops/spyddder-man"),
            extra_opts=extra_opts,
            ssh_opts=ssh_opts,
        )
    ...
    if node_type == "grq":
        ...
        rsync_project(
            "%s/ops/" % dir_path,
            os.path.join(ops_dir, "mozart/ops/grq2"),
            extra_opts=extra_opts,
            ssh_opts=ssh_opts,
        )
        ...
        rsync_project(
            "%s/ops/" % dir_path,
            os.path.join(ops_dir, "mozart/ops/tosca"),
            extra_opts=extra_opts,
            ssh_opts=ssh_opts,
        )
        ...
        rsync_project(
            "%s/ops/" % dir_path,
            os.path.join(ops_dir, "mozart/ops/pele"),
            extra_opts=extra_opts,
            ssh_opts=ssh_opts,
        )
...
def ensure_venv(
    ...
    if update_bash_profile:
        append(
            ".bash_profile",
            "source $HOME/{}/bin/activate".format(hysds_dir),
            escape=True,
        )
        append(
            ".bash_profile",
...
def install_pkg_es_templates():
    ...
    with prefix("source %s/bin/activate" % hysds_dir):
        run(
            "%s/ops/hysds_commons/scripts/install_es_template.sh %s" % (hysds_dir, role)
        )
...
def mozart_redis_flush():
    ...
    if ctx["MOZART_REDIS_PASSWORD"] != "":
        run(
            "redis-cli -a {MOZART_REDIS_PASSWORD} -h {MOZART_REDIS_PVT_IP} flushall".format(
                **ctx
            )
        )
    else:
...
def mozart_es_flush():
    ...
    run(
        "~/mozart/ops/hysds/scripts/clean_job_status_indexes.sh http://{MOZART_ES_PVT_IP}:9200".format(
            **ctx
        )
    )
    run(
        "~/mozart/ops/hysds/scripts/clean_task_status_indexes.sh http://{MOZART_ES_PVT_IP}:9200".format(
            **ctx
        )
    )
    run(
        "~/mozart/ops/hysds/scripts/clean_worker_status_indexes.sh http://{MOZART_ES_PVT_IP}:9200".format(
            **ctx
        )
    )
    run(
        "~/mozart/ops/hysds/scripts/clean_event_status_indexes.sh http://{MOZART_ES_PVT_IP}:9200".format(
            **ctx
        )
    )
...
def kill_hung():
    try:
        ...
        pass
    try:
        run("ps x | grep [s]flExec | awk '{print $1}' | xargs kill -TERM", quiet=True)
    except:
        pass
    try:
        run("ps x | grep [s]flExec | awk '{print $1}' | xargs kill -KILL", quiet=True)
    except:
        pass
    ps_x()
...
def verdid_clean_start():
    run(
        "rm -rf /data/work/scifloWork-ops/* /data/work/jobs/* /data/work/cache/* %s/verdi/log/*"
        % ops_dir
    )
    verdid_start(True)
...
def run_jenkins_cli(cmd):
    ...
    with prefix("source verdi/bin/activate"):
        run(
            "java -jar %s/war/WEB-INF/jenkins-cli.jar -s http://localhost:8080 -http -auth %s:%s %s"
...
def send_shipper_conf(
    ...
    if node_type == "mozart":
        ...
        upload_template(
            "job_status.template",
            "~/mozart/etc/job_status.template",
            use_jinja=True,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
            "worker_status.template",
            "~/mozart/etc/worker_status.template",
            use_jinja=True,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
            "task_status.template",
            "~/mozart/etc/task_status.template",
            use_jinja=True,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
        ...
        ctx.update({"cluster_jobs": cluster_jobs, "cluster_metrics": cluster_metrics})
        upload_template(
            "indexer.conf.metrics",
            "~/metrics/etc/indexer.conf",
            use_jinja=True,
            context=ctx,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
            "job_status.template",
            "~/metrics/etc/job_status.template",
            use_jinja=True,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
            "worker_status.template",
            "~/metrics/etc/worker_status.template",
            use_jinja=True,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
            "task_status.template",
            "~/metrics/etc/task_status.template",
            use_jinja=True,
            template_dir=os.path.join(ops_dir, "mozart/ops/hysds/configs/logstash"),
        )
        upload_template(
...
def send_queue_config(queue):
    ...
    upload_template(
        "install.sh",
        "~/verdi/ops/install.sh",
        use_jinja=True,
        context=ctx,
        template_dir=get_user_files_path(),
    )
    upload_template(
        "datasets.json.tmpl.asg",
    ...
    upload_template(
        ...
        "~/verdi/etc/supervisord.conf.tmpl",
        use_jinja=True,
        context=ctx,
        template_dir=get_user_files_path(),
    )
...
def rsync_sdsadm():
    ...
    rsync_project(
        "%s/ops/" % hysds_dir,
        os.path.join(ops_dir, "mozart/ops/sdsadm"),
        extra_opts=extra_opts,
        ssh_opts=ssh_opts,
    )
...
def conf_sdsadm(tmpl, dest, shared=False):
    ...
    upload_template(
        tmpl, dest, use_jinja=True, context=get_context(role), template_dir=tmpl_dir
    )
