...
def update_mozart(conf, ndeps=False, comp="mozart"):
    ...
    with tqdm(total=39) as bar:
        ...
        execute(
            fab.pip_install_with_req,
            "mozart",
            "~/mozart/ops/hysds/third_party/celery-v4.2.1",
            ndeps,
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.send_template_user_override,
            "supervisord.conf.mozart",
            "~/mozart/etc/supervisord.conf",
            "~/mozart/ops/hysds/configs/supervisor",
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.copy,
            "~/mozart/ops/hysds/configs/orchestrator/orchestrator_jobs.json",
            "~/mozart/etc/orchestrator_jobs.json",
            roles=[comp],
        )
        execute(
            fab.copy,
            "~/mozart/ops/hysds/configs/orchestrator/orchestrator_datasets.json",
            "~/mozart/etc/orchestrator_datasets.json",
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.cp_rp,
            "~/mozart/ops/hysds/scripts/job_creators",
            "~/mozart/etc/",
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.send_shipper_conf,
            "mozart",
            "/home/hysdsops/mozart/log",
            conf.get("MOZART_ES_CLUSTER"),
            "127.0.0.1",
            conf.get("METRICS_ES_CLUSTER"),
            conf.get("METRICS_PVT_IP"),
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.copy,
            "~/mozart/ops/mozart/configs/actions_config.json.example",
            "~/mozart/ops/mozart/actions_config.json",
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.ln_sf,
            "~/ssl/server.key",
            "~/mozart/ops/mozart/server.key",
            roles=[comp],
        )
        execute(
            fab.ln_sf,
            "~/ssl/server.pem",
            "~/mozart/ops/mozart/server.pem",
            roles=[comp],
        )
        bar.update()
        ...
        set_bar_desc(bar, "Updating HySDS core")
        execute(
            fab.pip_install_with_req, "verdi", "~/verdi/ops/osaka", ndeps, roles=[comp]
        )
        bar.update()
        execute(
            fab.pip_install_with_req,
        ...
        execute(
            fab.pip_install_with_req,
            "verdi",
            "~/verdi/ops/hysds/third_party/celery-v4.2.1",
            ndeps,
            roles=[comp],
        )
        bar.update()
        execute(
            fab.pip_install_with_req, "verdi", "~/verdi/ops/hysds", ndeps, roles=[comp]
        )
        bar.update()
        execute(
            fab.pip_install_with_req, "verdi", "~/verdi/ops/sciflo", ndeps, roles=[comp]
        )
        bar.update()
        execute(
            fab.pip_install_with_req,
        ...
        execute(
            fab.send_template_user_override,
            "supervisord.conf.verdi",
            "~/verdi/etc/supervisord.conf",
            "~/mozart/ops/hysds/configs/supervisor",
            roles=[comp],
        )
        bar.update()
...
def update_metrics(conf, ndeps=False, comp="metrics"):
    ...
    with tqdm(total=21) as bar:
        ...
        execute(
            fab.pip_install_with_req,
            "metrics",
            "~/metrics/ops/hysds/third_party/celery-v4.2.1",
            ndeps,
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.send_template_user_override,
            "supervisord.conf.metrics",
            "~/metrics/etc/supervisord.conf",
            "~/mozart/ops/hysds/configs/supervisor",
            roles=[comp],
        )
        bar.update()
        ...
        execute(
            fab.send_shipper_conf,
            "metrics",
            "/home/hysdsops/metrics/log",
            conf.get("MOZART_ES_CLUSTER"),
            conf.get("MOZART_PVT_IP"),
            conf.get("METRICS_ES_CLUSTER"),
            "127.0.0.1",
            roles=[comp],
        )
        ...
        execute(
            fab.send_template, "kibana.yml", "~/kibana/config/kibana.yml", roles=[comp]
        )
        bar.update()
...
def update_grq(conf, ndeps=False, comp="grq"):
    ...
    with tqdm(total=25) as bar:
        ...
        execute(
            fab.pip_upgrade, "gunicorn", "sciflo", roles=[comp]
        ...
        execute(
            fab.pip_install_with_req,
            "sciflo",
            "~/sciflo/ops/hysds/third_party/celery-v4.2.1",
            ndeps,
            roles=[comp],
        )
        bar.update()
        ...
        bar.update()
        execute(
            fab.pip_install_with_req, "sciflo", "~/sciflo/ops/grq2", ndeps, roles=[comp]
        )
        bar.update()
        execute(
            fab.pip_install_with_req,
        ...
        bar.update()
        execute(
            fab.pip_install_with_req, "sciflo", "~/sciflo/ops/pele", ndeps, roles=[comp]
        )
        bar.update()
        ...
        if os.path.exists(tosca_fv):
            execute(
                fab.copy,
                tosca_fv,
                "~/sciflo/ops/tosca/tosca/templates/facetview.html",
                roles=[comp],
            )
            execute(
                fab.chmod,
        ...
        execute(
            fab.send_template_user_override,
            "supervisord.conf.grq",
            "~/sciflo/etc/supervisord.conf",
            "~/mozart/ops/hysds/configs/supervisor",
            roles=[comp],
        )
        bar.update()
        ...
        # link ssl certs to apps
        execute(
            fab.ln_sf, "~/ssl/server.key", "~/sciflo/ops/grq2/server.key", roles=[comp]
        )
        execute(
            fab.ln_sf, "~/ssl/server.pem", "~/sciflo/ops/grq2/server.pem", roles=[comp]
        )
        execute(
            fab.ln_sf, "~/ssl/server.key", "~/sciflo/ops/tosca/server.key", roles=[comp]
        )
        execute(
            fab.ln_sf, "~/ssl/server.pem", "~/sciflo/ops/tosca/server.pem", roles=[comp]
        )
        execute(
            fab.ln_sf, "~/ssl/server.key", "~/sciflo/ops/pele/server.key", roles=[comp]
        )
        execute(
            fab.ln_sf, "~/ssl/server.pem", "~/sciflo/ops/pele/server.pem", roles=[comp]
        )
        bar.update()
        ...
        cont = (
            prompt(
                get_prompt_tokens=lambda x: [
                    (
                        Token.Alert,
                        "Updating component[s]: {}. Continue [y/n]: ".format(comp),
                    ),
                    (Token, " "),
                ],
        ...
        for queue in queues:
            ...
            with tqdm(total=5) as queue_bar:
                ...
                execute(
                    fab.cp_rp_exists,
                    "~/.netrc.verdi",
                    "~/verdi/ops/creds/.netrc",
                    roles=[comp],
                )
                execute(
                    fab.cp_rp_exists,
                    "~/.boto.verdi",
                    "~/verdi/ops/creds/.boto",
                    roles=[comp],
                )
                execute(
                    fab.cp_rp_exists,
                    "~/.s3cfg.verdi",
                    "~/verdi/ops/creds/.s3cfg",
                    roles=[comp],
                )
                execute(
                    fab.cp_rp_exists,
                ...
                execute(
                    fab.rm_rf,
                    "~/verdi/ops/beefed-autoindex-open_in_new_win.tbz2",
                    roles=[comp],
                )
                execute(
                    fab.copy,
                ...
                execute(
                    fab.rm_rf,
                    "~/code_configs/{}-{}.tbz2".format(queue, venue),
                    roles=[comp],
                )
                execute(
                    fab.ship_code,
...
def import_kibana(comp="metrics"):
    ...
    with tqdm(total=20) as bar:
        ...
        execute(
            fab.send_template,
            "import_dashboard.sh.tmpl",
            "~/metrics/ops/kibana_metrics/import_dashboard.sh",
            "~/mozart/ops/swot-pcm/conf/sds/files/kibana_dashboard_import",
            roles=[comp],
        )
        execute(
            fab.chmod,
        ...
        execute(
            fab.copy,
            "~/mozart/ops/swot-pcm/conf/sds/files/kibana_dashboard_import/job-dashboards.json",
            "~/metrics/ops/kibana_metrics/job-dashboards.json",
            roles=[comp],
        )
        execute(
            fab.copy,
            "~/mozart/ops/swot-pcm/conf/sds/files/kibana_dashboard_import/worker-dashboards.json",
            "~/metrics/ops/kibana_metrics/worker-dashboards.json",
            roles=[comp],
        )
        execute(
            fab.import_kibana,
        ...
        cont = (
            prompt(
                get_prompt_tokens=lambda x: [
                    (
                        Token.Alert,
                        "Updating Kibana: {}. Continue [y/n]: ".format(job_type),
                    ),
                    (Token, " "),
                ],
                validator=YesNoValidator(),
