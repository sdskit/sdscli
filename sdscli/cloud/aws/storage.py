...
def create_staging_area(args, conf):
    ...
    shasum = hashlib.sha224(
        "{}-{}-{}".format(bucket_name, args.prefix, args.suffix).encode()
    ).hexdigest()
    ...
    sns_client.set_topic_attributes(
        TopicArn=topic_arn, AttributeName="Policy", AttributeValue=json.dumps(pol)
    )
    sns_client.set_topic_attributes(
        TopicArn=topic_arn, AttributeName="DisplayName", AttributeValue=topic_name
    )
            ...
            "TopicConfigurations": [
                {
                    ...
                    "Filter": {
                        "Key": {
                            "FilterRules": [
                                {"Name": "prefix", "Value": args.prefix},
                                {"Name": "suffix", "Value": args.suffix},
                            ]
                        }
                    },
                }
    ...
    if args.debug:
        execute(
            func,
            "mozart/ops/hysds-cloud-functions/aws/data-staged",
            zip_file,
            roles=["mozart"],
        )
    else:
    ...
    cur_roles = OrderedDict(
        [(i["Arn"], i) for i in sorted(roles, key=itemgetter("CreateDate"))]
    )
    ...
    # create lambda
    function_name = "{}-dataset-{}-submit_ingest".format(
        conf.get("VENUE"), args.prefix.replace("/", "")
    )
    ...
    cf_args = {
        ...
        "VpcConfig": {"SubnetIds": subnets, "SecurityGroupIds": sgs},
        "Environment": {
            "Variables": {
    ...
    sns_resp = sns_client.subscribe(
        TopicArn=topic_arn, Protocol="lambda", Endpoint=lambda_resp["FunctionArn"]
    )
