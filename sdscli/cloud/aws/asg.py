...
from sdscli.prompt_utils import (
    YesNoValidator,
    SelectionValidator,
    MultipleSelectionValidator,
    Ec2InstanceTypeValidator,
    PriceValidator,
    highlight,
    ...
    for i, x in enumerate(ids):
        ...
        pt.append(
            (
                Token,
                ". {} - {} ({})\n".format(
                    images[x]["Name"], x, images[x]["CreationDate"]
                ),
            )
        )
...
def prompt_secgroup(sgs, desc=None):
    ...
    while True:
            ...
            map(
                ...
                [
                    ...
                    for i in prompt(
                        get_prompt_tokens=lambda x: pt,
                        style=prompt_style,
                        validator=MultipleSelectionValidator(),
                    ).split()
                ],
            )
        )
        ...
        if len(vpc_ids) > 1:
            print(
                (
                    "Invalid selections. Security groups from multiple VPC IDs selected: {}".format(
                        list(vpc_ids)
                    )
                )
            )
...
def create(args, conf):
    ...
    cur_images = OrderedDict(
        [
            ...
            for i in [
                ...
                for x in sorted(
                    get_images(
                        c=ec2, Filters=[{"Name": "is-public", "Values": ["false"]}]
                    ),
    ...
    instance_types = (
        conf.get("INSTANCE_TYPES").split() if "INSTANCE_TYPES" in conf._cfg else None
    )
    instance_bids = (
        conf.get("INSTANCE_BIDS").split() if "INSTANCE_BIDS" in conf._cfg else None
    )
    for i, queue in enumerate([i.strip() for i in conf.get("QUEUES").split()]):
        ...
        if instance_types is None:
            instance_type = prompt(
                ...
                style=prompt_style,
                validator=Ec2InstanceTypeValidator(),
            ).strip()
        else:
        ...
        if instance_bids is None:
            ...
            if use_spot:
                market = "spot"
                spot_bid = prompt(
                    get_prompt_tokens=lambda x: [(Token, "Enter spot price bid: ")],
                    style=prompt_style,
        ...
        asg_args = {
            ...
            "Tags": [
                ...
                {"Key": "Venue", "Value": conf.get("VENUE"), "PropagateAtLaunch": True},
                {"Key": "Queue", "Value": queue, "PropagateAtLaunch": True},
            ],
        }
        ...
        logger.debug(
            "Target tracking scaling policy {}: {}".format(
                policy_name, pformat(ttsp_info)
            )
        )
        print(
            ("Added target tracking scaling policy {} to {}".format(policy_name, asg))
        )
